 <?php
	...
	if(isset($_POST['sub'])){
		...
		if(isPaied($pid,$dat) == false){
			$qry = "insert into payment set pid = '$pid', name = '$name', loc = '$loc', contact = '$contact', dat = '$dat', taka = '$paymt', status = '$hd'";
			mysql_query($qry);
...
?>
	...
	<table align="center" border="0" width="330" cellspacing="0" style="font-family:'Microsoft JhengHei'; font-size:13px; text-align:left">
		...
		<form action="payment_01.php" method="post">
		<input type="hidden" name="pid" id="pid" value="<? echo $pid; ?>" />
		<table align="center" border="0" width="330" cellspacing="0" style="font-family:'Microsoft JhengHei'; font-size:13px; text-align:left">
		<tr>
			<td width="100">Name</td>
			<td width="1">:</td>
			<td><input type="text" name="name" id="name" value="<? echo $name; ?>" style="background-color:white"/></td>
		</tr>
		
		<tr>
			<td>Contact</td>
			<td>:</td>
			<td><input type="text" name="contact" id="contact" value="<? echo $contact; ?>" style="background-color:white"/></td>
		</tr>
		
		<tr>
			<td>Location</td>
			<td>:</td>
			<td><input type="text" name="location" id="location" value="<? echo $loc; ?>" style="background-color:white"/></td>
		</tr>
		...
		<tr>
			<td>Date</td>
			<td>:</td>
			<td><input type="text" name="date" id="date" value="<? echo date("Y-m-d"); ?>" readonly style="background-color:gray; color:#CCCCCC"/></td>
		</tr>
